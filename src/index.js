...
const shapeReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case ActionTypes.SET_OPACITY: {
      ...
      return state.merge({
        allShapes: state.allShapes.update(id, merge(() => ({ opacity: Math.round(opacity * 100) / 100 })))
      })
    }
    case ActionTypes.BRING_TO_FRONT: {
      const { allShapes, selectedShapeIds } = state
      const shape = allShapes[selectedShapeIds[0]]

      return state.merge({
        allShapes: allShapes.update(shape.parentId, merge(({ childIds }) => ({
